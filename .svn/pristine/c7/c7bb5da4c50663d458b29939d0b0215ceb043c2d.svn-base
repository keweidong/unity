...
namespace DashFire
{
  public class SkillStateInfo
  {
    ...
    public int GetSkillAppendScore()
    {
      int skill_append_score = 0;
      if (null != m_SkillList && m_SkillList.Count > 0) {
        foreach (SkillInfo skill in m_SkillList) {
          if ((int)skill.Postions.Presets[0] > 0) {
            SkillLogicData skill_data =
              SkillConfigProvider.Instance.ExtractData(SkillConfigType.SCT_SKILL, skill.SkillId) as SkillLogicData;
            if (null != skill_data)
    ...
    {
      ImpactInfo info;
      m_ImpactListForCheck.TryGetValue(impactId, out info);
      return info;
    }
    ...
    public void CleanupImpactInfoForCheck(long curTime, long additionalLifeTime)
    {
      ...
      {
        m_ImpactListForCheck.Remove(deletes[i]);
      }
      /*
