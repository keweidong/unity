...
namespace DashFire.Story.Commands
{
  ...
  internal class MissionCompletedCommand : AbstractStoryCommand
  {
    ...
    protected override bool ExecCommand(StoryInstance instance, long delta)
    {
      LogSystem.Debug("missioncompleted");
  ...
  internal class ChangeSceneCommand : AbstractStoryCommand
  {
    ...
    protected override bool ExecCommand(StoryInstance instance, long delta)
    {
      int mainSceneId = m_MainSceneId.Value;
      if (mainSceneId <= 0) {
        mainSceneId = LobbyClient.Instance.CurrentRole.CitySceneId;
      }
      WorldSystem.Instance.QueueAction((MyFunc<int, bool>)WorldSystem.Instance.ChangeScene, mainSceneId);
      return false;
    }
