...
namespace GfxModule.Skill.Trigers
{
  ...
  public class SkillDamageManager
  {
    ...
    public void SendImpactToObject(GameObject source, GameObject target, Dictionary<BeHitState, StateImpact> stateimpacts, int skillid, long hit_count_id)
    {
      ...
      {
        DashFire.LogicSystem.NotifyGfxHitTarget(damageOwner, stateimpact.m_Impacts[i].ImpactId, target, 1, final_skill_id,
      ...
      foreach (ImpactData im in stateimpact.m_Impacts) {
        DashFire.LogicSystem.NotifyGfxHitTarget(damageOwner, im.ImpactId, target, 1, final_skill_id, 
    ...
    public void SendImpactToObject(GameObject source, GameObject target, int impactid, int remaintime, int skillid, long hit_count_id)
    {
      ...
      DashFire.LogicSystem.NotifyGfxHitTarget(damageOwner, impactid, target, 1, final_skill_id,
  ...
  internal class AreaDamageTriger : AbstractSkillTriger
  {
    ...
    public override bool Execute(object sender, SkillInstance instance, long delta, long curSectionTime)
    {
      if (curSectionTime >= m_StartTime) {
        ...
        int character_layer = 1 << LayerMask.NameToLayer("Player") | 1 << LayerMask.NameToLayer("Monster");
        Collider[] hits = Physics.OverlapSphere(center, m_Range, character_layer);
