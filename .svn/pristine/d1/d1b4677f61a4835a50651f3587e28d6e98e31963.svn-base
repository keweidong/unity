...
namespace DashFireSpatial
{
  public sealed class SpatialSystem : ISpatialSystem
  {
    ...
    {
      if (!GlobalVariables.Instance.IsClient) {//客户端暂时不读避让信息（xml库在ios上有问题。。）
	    float mapwidth = cell_manager_.GetMapWidth();
	    float mapheight = cell_manager_.GetMapHeight();
	    MapParser mapParser = new MapParser();
	    mapParser.ParseTiledData(DashFire.HomePath.GetAbsolutePath(file), mapwidth * scale, mapheight * scale);
    ...
    public bool CheckCollide(ISpaceObject hiter, ISpaceObject dest)
    {
      ...
      if (is_collide) {     // 碰撞
        if (!is_allready_collide) {  // 之前没有碰撞,记录下相撞的物体
          //LogSystem.Debug("{4} spatial collide obj({0},{1}) with obj({2},{3})",
        ...
        //LogSystem.Debug("{4} spatial depart obj({0},{1}) with obj({2},{3})",
