...
namespace DashFire
{
  public class CombatStatisticInfo
  {
    ...
    public long HitCountBreakTime
    {
      get { return m_LastHitTime + m_HitCountRemainTime; }
    }
    ...
    public bool UpdateMultiHitCount(long hit_count_id, int hitCount, long curTime)
    {
      bool isMaxHitCountChanged = false;
      if (0 != m_LastHitTime && HitCountBreakTime < curTime) {
        m_MultiHitCount = 0;
      }
      if (hitCount <= 0) {
        return false;
      }
      if (m_LastHitCountId != hit_count_id) {
        m_MultiHitCount = m_MultiHitCount + hitCount;
        m_LastHitCountId = hit_count_id;
        m_LastHitTime = curTime;
      }
      if (MaxMultiHitCount < MultiHitCount) {
        MaxMultiHitCount = m_MultiHitCount;
        isMaxHitCountChanged = true;
      }
      return isMaxHitCountChanged;
    }
    ...
    private long m_HitCountRemainTime = 1500;
