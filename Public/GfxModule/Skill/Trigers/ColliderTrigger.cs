...
namespace GfxModule.Skill.Trigers
{
  ...
  internal class ColliderDamageTriger : AbstractSkillTriger
  {
    ...
    private void UpdateMoreTimeEffectObjects(SkillInstance instance)
    {
      ...
      for (int i = 0; i < m_MoreTimesEffectObjects.Count; i++)
      {
        if (m_MoreTimesEffectObjects[i].NextEffectTime <= now)
        {
          ...
          m_DamageManager.SendImpactToObject(m_DamageManager.GetOwner(), m_MoreTimesEffectObjects[i].TargetObj, m_ColliderInfo.GetStateImpacts(), instance.SkillId, m_CurHitCountId);
        }
      }
      /*
      foreach (EffectObjectInfo obj_info in m_MoreTimesEffectObjects) {
        if (obj_info.NextEffectTime <= now) {
          ...
          m_DamageManager.SendImpactToObject(m_DamageManager.GetOwner(), obj_info.TargetObj, m_ColliderInfo.GetStateImpacts(), instance.SkillId, m_CurHitCountId);
        }
      }*/
    }
    ...
    private void UpdateHitCount()
    {
      ...
      long now = m_OwnSkill.CurTime;
      if (m_LastHitCountTime + TriggerUtil.MinHitCountCD() > now) {
        m_CurHitCountId = TriggerUtil.NextHitCountId();
        m_LastHitCountTime = now;
      }
    }
...
//       }
    ...
    private long m_CurHitCountId = 0;
    private long m_LastHitCountTime = 0;
  }
}
