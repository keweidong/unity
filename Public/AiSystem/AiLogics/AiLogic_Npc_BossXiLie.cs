...
namespace DashFire {
  class AiLogic_Npc_BossXiLie : AbstractNpcStateLogic {
    ...
    private void PursuitHandler(NpcInfo npc, AiCommandDispatcher aiCmdDispatcher, long deltaTime)
    {
      ...
      AiData_Npc_BossXiLie data = GetAiData(npc);
      if (null != data) {
        ...
        if (null != target) {
          ...
          if (null != data.CurSkillCombo) {
            if (data.CurSkillCombo.Length > data.CurSkillComboIndex) {
              ...
              if (data.IsInDecontrolCombo) {
                if ((int)GfxCharacterState_Type.HitFly== target.GfxStateFlag) {
                  data.CurSkillCombo = data.FlyCombo;
                } else {
                  data.CurSkillCombo = data.StandCombo;
                }
              } else {
          ...
          if (powDist > m_AttackRange * m_AttackRange) {
            ...
            if (IsCanUseComboSkills(npc, target, data.MeeleCombo)) {// 近战连招
              data.CurSkillCombo = data.MeeleCombo;
              data.CurSkillComboIndex = 0;
            } else if(TryCastSkill(npc, data.MeeleSkill, target)) { // 默认释放技能
              return;
            }
          }
        } else {
    ...
    private AiData_Npc_BossXiLie GetAiData(NpcInfo npc)
    {
      AiData_Npc_BossXiLie data = npc.GetAiStateInfo().AiDatas.GetData<AiData_Npc_BossXiLie>();
      if (null == data) {
        data = new AiData_Npc_BossXiLie();
