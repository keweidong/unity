...
namespace DashFire
{
  ...
  public class ItemConfig : IData
  {
    ...
    public int m_ExperienceProvide = 0;
    public int m_UseLogicId = 0;
    public string[] m_UseResultData = null;
    public string[] m_UseIconOnLevel;           //"level|icon[|level|icon...]
    public string[] m_UseModelOnLevel;          //"level:model[|level:model...]
    public int[] m_ActiveBuffOnLevel = null; //"level|buffid[|level|buffid...]
    ...
    public List<int> m_ItemSourceScene = new List<int>();
  ...
  public class ItemRouteConfig : IData
  {
    public int Id;
    public int WarriorItemId;
    public int RogueItem;
    public int MageItem;
    public int PaladinItem;
    public int HunterItem;
    public int ShamanItem;
    public int DruidItem;
    public int WarlockItem;
    public int PriestItem;

    public bool CollectDataFromDBC(DBC_Row node)
    {
      Id = DBCUtil.ExtractNumeric<int>(node, "Id", 0, true);
      WarriorItemId = DBCUtil.ExtractNumeric<int>(node, "WarriorItem", 0, false);
      RogueItem = DBCUtil.ExtractNumeric<int>(node, "RogueItem", 0, false);
      MageItem = DBCUtil.ExtractNumeric<int>(node, "PaladinItem", 0, false);
      HunterItem = DBCUtil.ExtractNumeric<int>(node, "HunterItem", 0, false);
      ShamanItem = DBCUtil.ExtractNumeric<int>(node, "ShamanItem", 0, false);
      DruidItem = DBCUtil.ExtractNumeric<int>(node, "DruidItem", 0, false);
      WarlockItem = DBCUtil.ExtractNumeric<int>(node, "WarlockItem", 0, false);
      PriestItem = DBCUtil.ExtractNumeric<int>(node, "PriestItem", 0, false);
      return true;
    }
  ...
  {
    public DataDictionaryMgr<ItemConfig> ItemConfigMgr
    {
    ...
    public int GetReliveStoneId()
    {
      return m_ReliveStoneId;
    }
    public int GetSweepStageItemId()
    {
      return m_SweepStageItemId;
    }
    public int GetMonthCardId()
    {
      return m_MonthCardId;
    }
    ...
    public void Clear()
    {
      m_ItemConfigMgr.Clear();
    }

    private const int m_GoldId = 101;
    private const int m_DiamondId = 102;
    private const int m_ReliveStoneId = 104;
    private const int m_SweepStageItemId = 105;
    private const int m_MonthCardId = 106;
    private const int m_ExchangeCurrencyId = 200001;
    private DataDictionaryMgr<ItemConfig> m_ItemConfigMgr = new DataDictionaryMgr<ItemConfig>();
    private DataDictionaryMgr<ItemRouteConfig> m_ItemRouteConfigMgr = new DataDictionaryMgr<ItemRouteConfig>();
